...
A *trigger* is an event that starts a flow. For example, **When a new email arrives (V2)** is a trigger that starts a flow when an email arrives in an inbox. For information about how to trigger flows based on the properties of an email, go to [Trigger a flow based on email properties](https://docs.microsoft.com/power-automate/email-triggers).
