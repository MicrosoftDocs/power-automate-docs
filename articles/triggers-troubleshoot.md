...
1. There has been too many calls to the connector or flow, causing it to be throttled. To verify if your flow is being throttled, manually test the flow to see if it triggers right immediately If it triggers immediately, it is not throttled.
...
1. Fix/update connections.
   ...
   If your flow is uses a **Manual** trigger, it needs the connection of the user who is triggering the flow. If it uses the **Recurrence** trigger, it can run on the flow maker's connections.
