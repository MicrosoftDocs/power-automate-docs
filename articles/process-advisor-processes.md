---
title: Prepare processes and recordings in process advisor (Video) | Microsoft Docs
...
1. On the **Process** screen, select the process you want to record from the dropdown.
...
1. By using any of the methods described above, you should receive a message that says **Launching the recorder in Power Automate**.

    > [!NOTE]
    > You should have [downloaded Power Automate](https://go.microsoft.com/fwlink/?linkid=2102613) before you started. However, you can also select **Get the app** to install it.
...
1. Once the recording has been saved successfully, select **Got it** to close the message.
...
Once the recording has completed, you can view the recorded actions on the recording details screen. More importantly, you need to prepare the recording to be ready for analysis. What does this mean?

- The actions recorded can be granular. Group them into activities. These will be the building blocks for the process map that's created through analysis.
...
1. When you finish grouping, set **Ready to analyze** on the top right.
...
As a process owner or co-owner, you can create recommended activity names for a process. This will ensure more consistency in naming across recordings. To do this:
