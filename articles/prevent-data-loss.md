...
|July 2022 | Enforce DLP policies on child flows | Enable the enforcement of DLP policies to include child flows. If a violation is found anywhere in the flow tree, the parent flow is suspended. After editing the child flow and saving to remove the violation, the parent flows can be resaved to run the DLP policy evaluation again. A change to no longer block child flows when the HTTP connector is blocked will roll out along with hard enforcement of DLP policies on child flows. | Learning |October 10, 2022|December|
