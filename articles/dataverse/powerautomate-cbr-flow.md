...
The Callback Registration record is stored in Dataverse as a `CallbackRegistration` entity and contains metadata about the trigger configuration:

| Field | Description |
|-------|-------------|
| Entity name | Target table (for example, "account", "contact") |
| Message type | Event type (Create, Update, Delete, or custom action) |
| Filtering attributes | Specific columns to monitor |
| Filter expression | Conditional rules for triggering |
| Scope | User, Business Unit, or Organization |
| Owner | Owner of the subscription |
| Endpoint URL | Power Automate callback URL |
| Logic Apps Version | Version information |

This metadata allows Dataverse to subscribe to table events and call the correct flow with the appropriate context.
