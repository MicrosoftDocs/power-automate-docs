---
title: Create an Object‑centric event log
...
Creating an object‑centric event log (OCEL) involves translating a real‑world process into a structured dataset that accurately reflects the relationships between events and business objects. Unlike traditional case‑centric logs, this approach captures the complexity of scenarios where multiple objects interact within the same event. The goal is to produce a clean, consistent, and analysis‑ready log that supports object‑centric process mining without unnecessary complexity.
...
> [!NOTE]
> The current version supports data ingestion *from a CSV file stored in Azure Data Lake or OneLake* or *delta table stored in the Fabric Lakehouse*. Ensure your final log is exported to one of these locations before proceeding with ingestion.
...
- `Actor`: Person or system executing the event.
- **One column per object type** used in your process (for example, `Order`, `Invoice`, `Payment`, …).
  - If an event touches *multiple* objects of the same type, separate them with `|` (for example, `O1|O2`).
...
- **Avoid attribute duplication**: Keep object attributes (for example, `PaymentAmount`) once per event row if that object is present. If multiple same‑type objects must carry different attribute values, split them into separate events.
...
2. **Inventory source events**<br>
   Identify upstream event sources (tables, logs, or status changes) and map each source record to
...
4. **Normalize attributes**<br>
   Standardize timestamps, user IDs, currencies, and units; keep attributes only if they’re analytically useful.
5. **Emit CSV**<br>
...
The following steps 1 through 4 describe the process.
...
Keep this list minimal. Extend it only if more objects materially change your analysis.

#### 2. Gather relevant events for each object (event streams before merging)
...
> At this stage, keep each stream separate (one CSV per object type) or use staging tables or views.
...
Concatenate all event rows and **retain the shared column set**. The following sections describe three variants:
...
- **Object‑level attribute**: An attribute that describes an object (for example, *PaymentAmount* for *Payment `P1`*). You can repeat these attributes at event time when the event references that object.
- **Object type**: A category or class of business entities (for example, *Order*, *Invoice*, *Payment*, *SupplierOrder*).
