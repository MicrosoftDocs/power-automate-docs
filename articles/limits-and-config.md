...
|Premium flows without premium licenses| 14 days | Flows previously created using premium features (premium connectors/custom connectors/HTTP connectors/On premise gateway/Business process flows) but donot have a premium Power Automate license will be turned off in 14 days. This can happen if the original owner left the company or has an expired trial/premium license. The flow owner and co-creators will get an email when the trial/license expired or when the owner is not found in AAD.The flow will continue to work for 14 days. If no action is taken in next 14 days, the flow will be systematically turned off and the owner and co-creators will be notified in an email. Newly created/edited premium flows without a premium license will be saved but turned off. Once a premium Power Automate license is assigned to the owner/flow, the flow can be turned on. Admins can find such flows using the latest version of [Center of Excellence (CoE) Starter Kit](https://docs.microsoft.com/en-us/power-platform/guidance/coe/starter-kit). Assign a per flow license or a per user license to the owner to keep the flow running.|
|Power Apps out of context flows| 90 days|Power Apps licenses include a limited set of Power Automate capabilities that allow the users to run flows that are in context of the Power App.See [Power Automate use rights included with Power Apps licenses section in Microsoft Power Platform Licensing Guide for details](https://docs.microsoft.com/en-us/power-platform/admin/power-automate-licensing/faqs#what-power-automate-capabilities-are-included-in-power-apps-licenses). Flows triggered from a Power App are automatically considered in context. For automated/scheduled cloud flows created to support the app and in context of the app, link the flow to the app using a [PowerShell script](https://docs.microsoft.com/en-us/power-platform/admin/powerapps-powershell#associate-in-context-flows-to-an-app).Owners and co-owners of premium automated/scheduled flows previously created will get email notifications and have 90 days to purchase a license. If no action is taken, the flows will be turned off after 90 days.Admins can find such flows using the latest version of [Center of Excellence (CoE) Starter Kit](https://docs.microsoft.com/en-us/power-platform/guidance/coe/starter-kit). Assign a per flow license or a per user license to the owner to keep the flow running. Newly created/edited premium flows without a premium license will be saved but turned off. Once a premium Power Automate license is assigned to the owner/flow, the flow can be turned on.|
