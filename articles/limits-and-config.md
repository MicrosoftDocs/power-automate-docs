...
|Power Apps out of context flows| 90 days|Power Apps licenses include a limited set of Power Automate capabilities that allow the users to run flows that are in context of the Power App.See [Power Automate use rights included with Power Apps licenses section] (https://docs.microsoft.com/en-us/power-platform/admin/power-automate-licensing/faqs#what-power-automate-capabilities-are-included-in-power-apps-licenses) for details. Owners and co-owners of premium automated/scheduled flows previously created and not linked to an app will get email notifications and have 90 days to purchase a license. If no action is taken, the flows will be turned off after 90 days.Admins can find such flows using the latest version of [Center of Excellence (CoE) Starter Kit](https://docs.microsoft.com/en-us/power-platform/guidance/coe/starter-kit). Assign a per flow license or a per user license to the owner to keep the flow running. Newly created/edited out of context premium flows without a premium license will be saved but turned off. Once a premium Power Automate license is assigned to the owner/flow, the flow can be turned on.|
