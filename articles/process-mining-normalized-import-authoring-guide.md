...
>
> If your source is **Fabric**, there's a community widget that gives you a more user-friendly UI for generating the JSON: [Normalized Schema Generator](https://github.com/rosikm/normalized-schema-generator).

## When to use normalized import

Use **normalized import** when your source data already lives as multiple related tables&mdash;for example, Delta tables or files in OneLake/Fabric Lakehouse or files in ADLS&mdash;and you want Process Mining to read them as-is.
...
### `ExportName`&mdash;what it is and when to set it
...
|---|---|---|---|
| `dataSource` | object | yes | Physical layout. More information: [`dataSource`](#datasource) |
| `miningMetadata.ImportConfiguration` | object | yes | Logical model. More information: [`miningMetadata.ImportConfiguration.Attributes[]`](#miningmetadataimportconfigurationattributes) |
...
| `azureDataLakeConnectionSetupProperties` | object | required when `dataSourceType=1` | More information: [`azureDataLakeConnectionSetupProperties` (ADLS Gen2)](#azuredatalakeconnectionsetupproperties-adls-gen2) |
| `oneLakeConnectionSetupProperties` | object | required when `dataSourceType=2` | More information: [`oneLakeConnectionSetupProperties` (OneLake / Fabric)](#onelakeconnectionsetupproperties-onelake--fabric) |
...
- [ ] Event-dataset joins point only at Case or Join targets&mdash;no self-reference, no Event-to-Event.
...
`Start` and `End` attributes must come from a column physically present on the Event dataset (a direct `Columns` entry)&mdash;not pulled in through a join. Otherwise the validation isn't passed.

### Missing `Join: null`

The `Join` property is required on every dataset. When a dataset has no joins, set `"Join": null` explicitly&mdash;omitting the property is a shape error.

### Mismatched attribute name

Every `Attributes[].Name` must resolve to a physical column or join export somewhere in `datasets`. If it doesn't, validation fails. Remember: when a join sets `ExportName`, the FK is surfaced under that name&mdash;not under `SourceColumnName`.
