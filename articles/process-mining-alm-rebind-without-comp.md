...
1. If you didn't newly import the process, go to step 6.

    If you did newly import the process, go to the next step.
...
1. (If you skipped step 4) Attach a custom Power BI workspace to the process:
    ...
    1. Provide the same report name as the name you used for the process in Dev environment.
        ...
        > [!IMPORTANT]
        >
        > The report name must be identical to the process nanme in Dev environment.

    1. Select **Save** > **Continue**.
    1. From the command bar or in the report view, select **Refresh data**.

### Edit and rebind different workspaces
...
1. To publish from the command bar, select **Save** > **Publish**.
1. From the **Publish to Power BI** menu, select your **WkSpace B** > **Select**.

    A message that says **Replacing report** appears to indicate you retained the report name.

1. To overwrite the old report in WkSpace B, select **Replace**. This action overwrites the existing report with an updated version that incorporates the new customizations. It also preserves the original report name for consistency.
...
1. When this process completes, return to Power BI and open **WkSpace B**.
    ...
    The report is now successfully transferred from WkSpace A and is connected to the semantic model of the process you had in your WkSpace B.
...
1. To find your process, do steps 1 through 3 in [Select your workspaces](#select-your-workspaces) in this article.
1. Select a new workspace for this process by following these steps:

    1. In the **Power BI workspace** dropdown menu, select the *same* workspace as the previous workspace.
    1. In the **Report Name** field, enter a different report name as your previous report.

        > [!IMPORTANT]
        > The report name must be different from your previous report.

    1. To continue setting up the process, select **Continue**.

       If you want an example of the screen, go to step 5c in [Select your workspaces](#select-your-workspaces) in this article.

1. If you didn't newly import the process, go to step 5.

    If you did newly import the process, go to the next step.

1. (If you skipped step 3 in this section) Attach a custom Power BI workspace to the process:
    1. Go to the process details page for the process.
    1. On the **Details** card, select **Edit**.
    1. From the **Power BI workspace (optional)** dropdown menu, select your **WkSpace A** Power BI workspace that you want to port your changes to.

        > [!IMPORTANT]
        > In the **Report Name field, enter a name different from the report name of your previous report.
  
    1. Select **Save** > **Continue**.
    1. From the command bar or in the report view, select **Refresh data**.

## Edit and rebind the same workspace

Some of the steps in the following procedure are the same as the steps for editing and rebinding different workspaces. These steps guide you to the preiously described steps with a link. As you complete the steps, return here to complete editing and rebinding the same workspace.

1. To download your report in Power BI, do steps 1 through 5 in [Edit and rebind different workspaces](#edit-and-rebind-different-workspaces).
...
1. From the list of destinations, select **WkSpace A** > **Select**.
    ...
    A message that says **Replacing report** appears to indicate you gave the report the same name as your old Report B.
...
1. When this process completes, return to Power BI and open **WkSpace A**.
    ...
    The customizations from Report A are now successfully transferred to Report B within the same **WkSpace A**.