...
- The **Owner** role (on a storage account level) must be attributed to the user performing the initial container setup (user then becomes the Owner of the process). In addition, all users that have the **Owner** or **Co-Owner** role on the process must be attributed:
