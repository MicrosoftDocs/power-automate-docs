...
When a patch is available, machines upgrade to the patched minor version (2.XX.Y to 2.XX.Z) or to the latest version with the patch, which may include a minor version update (2.X to 2.Y). Machines only upgrade when Power Automate for Desktop is not in use.
...
The only way to configure automatic updates is through the Windows registry. To change the default behavior, create the following registry values. 
