...
|---|---|---|---|  
| Automatic Data Retention | Not Available | Available | V2 leverages the [Elastic Tables](/power-apps/maker/data-platform/create-edit-elastic-tables) which is powered by Azure Cosmos DB and comes with a built-in time-to-live feature for automatic data retention. |  
| Support for large log sizes | Up to 32MB | Initially 64MB | V2 has a theoretical limit of 20GB per desktop flow run to which we could potentially scale up to in future. |  
...
| Support for Azure Synapse link for Dataverse integration | Not Available | Available | In V1, the attribute 'additionalcontext' is a file type, stored as a blob in Dataverse, which is not supported for synchronization to Azure Synapse. Logs V2 can be synced to Azure Synapse. |  
| Support for Dataverse auditing | Not Available | Available | In V1, the attribute 'additionalcontext' is a file type, stored as a blob in Dataverse, which is not supported in Dataverse auditing. Logs V2 supports Dataverse auditing. |  
...
| **V2 - Stored in the FlowLogs entity (Preview)** | Logs are stored in the 'FlowLogs' entity which is backed by [Elastic Tables](/power-apps/maker/data-platform/create-edit-elastic-tables). Logs V2 consumes Dataverse Database capacity which is much more performant and accessible but also significantly more expensive than file based capacity. |  
...
The following table shows sample storage consumption estimates per desktop flow run when using Logs V2. It outlines the approximate storage demand for different numbers of actions, assuming an average of 3KB storage per action.
...
The following API call retrieves a specific flow session by its ID (9d51aa1f-315e-43ab-894f-bc445dfb049b), and then access the associated action logs using the *flowsession_flowlog_parentobjectid* relationship.
...
##### Breaking the call down into individual pieces
...
- The parameters for the ExecuteCosmosSqlQuery method are provided in parentheses following the method name. These are:
  ...
  - EntityLogicalName=@p2: This is the logical name of the entity you are querying. Here, it's set to 'flowlog'.
...
- Cascade delete is not yet supported. Therefore, it's recommended to avoid removing TTL.
