...
- Power Automate for desktop is fully backwards compatible but forward compatibility is not guaranteed. Each update may introduce action upgrades that change their signature (properties or values of an action) and/or a change in the engine powering runtime and authoring. Trying to run or edit a desktop flow created with a newer version of Power Automate for desktop may result in the following error message:
...
To resolve this issue edit and run the flow with the version that was used to create/ edit it or use a newer version of Power Automate for desktop.
