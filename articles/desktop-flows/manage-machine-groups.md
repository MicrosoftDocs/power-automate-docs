...
When generating a machine group password, the group's keys are encrypted into a PFX file using the AES256 algorithm.

Windows Server 2016 does not support exporting or importing PFXs protected with AES256 and uses 3DES instead, a deprecated encryption algorithm.
When a group password has been generated by a more recent version of Windows, Windows Server 2016 machines won't be able to join that group. 

To allow a Windows Server 2016 to join a group of machines which has at least one machine running a later version of Windows, you must export the group keys in a format that Windows Server 2016 understands by generating the password in legacy mode.

> [!IMPORTANT]
> 3DES is a deprecated encryption algorithm. Using the machine group feature with Windows Server 2016 weakens your group's security and is not recommended.

1. Connect to a machine that is already a member of the group.
1. Navigate to the `Computer\HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Power Automate Desktop\Global` registry key.
1. Create a DWORD value named `UseLegacyPasswordProtectionForGroupKey` and set it to 1.
1. Re-generate the group password using the Power Automate machine runtime application.
1. Join the Windows Server 2016 machine to the group using the newly generated password.
1. RECOMMENDED: Delete the registry value created above and re-generate the group password again from a non-Windows Server 2016 machine, so that the keys are protected with AES256 after you are done joining Windows Server 2016 machines to the group.
