...
| Choices | Yes | Title and Value || Describes choices for use in the choise set. Consists of a title (text to display) and a value (raw value for the choice) |
...
| Height in pixels | No | Numeric value | 0 | The desired height of the image. The image will distort to fit that exact height. This overides the 'Size' property |
| Horizontal alignment | N/A | Left, Center, Right | Left | Controls how this element is horizontally positioned |
| ID | No | Text value | Depends on the element | Unique idetifier for the value. Used to identify collected input when the Submit action is performed. If an invalid ID is temporarily used and the form designer is closed, the last valid ID used will apply |
...
| Strikethrough | N/A | Boolean value | False | If enabled, crosses the text out |
...
| Subtle | N/A | Boolean value | False | If enabled, displates text slightly toned down to appear less prominent |
