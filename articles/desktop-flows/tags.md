...
> [!NOTE]
> Users with the **Environment maker** role can access and use all tags within an environment by default, including those created by others. Explicit sharing with other users isn't needed. <!-- TO BE ADDED HERE WHEN THE BEHAVIOR IS FIXED IN v.2.57: They can associate the required tags with flows they own or co-own, but not flows shared with them as run-only users.-->
