...
- The Power Automate portal doesn’t currently surface additional Self-healing details in the action logs of the flow runs where self-healing was invoked. As a workaround, you will be able to use this [registry setting](https://learn.microsoft.com/power-automate/desktop-flows/governance#configure-power-automate-for-desktop-to-prevent-cleanup-of-flow-run-action-details) when needed, so that you can easily retrieve action logs, including the Self-healing activity, through local log files in the machine that hosted the flow run. In the action logs, a new property called *repairWithAIInfo* displays if a repair attempt was made, whether it was successful or failed, and the AI-generated selector that was used at runtime.
