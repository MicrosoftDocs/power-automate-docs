...
7. (Optional) Activate SLA strategy if you want to enable Work queues SLA. Select the default time-to-live (TTL) and configure the default SLA violation in the **SLA violated after** field. Then, determine the the **SLA considered at risk after (preview)** configuration. The values for the slider is based on the **SLA violation after** configuration.
   > [!NOTE]
