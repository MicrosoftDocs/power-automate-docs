...
|File(s) to zip|No|[List of FileSystemObject](../variable-data-types.md#list)||The full path of the file(s) or folder(s) to include in the ZIP file|
...
|Destination folder|No|[Folder](../variable-data-types.md#files-and-folders)||The full path of the folder to extract the archive to. This overwrites files in the folder with the same name as a file in the archive|
...
|Include mask|Yes|[Text value](../variable-data-types.md#text-value)||The filter to limit the files extracted to those entered here. This allows wild cards, for example '*.txt' or 'document?.doc' (without the quotes). To allow multiple file filters, separate the choices with a semi-colon, for instance, '*.txt;*.exe'|
