...
- To use premium connectors, you need an Attended RPA license. If you have a free work or school account, you can access standard connectors.
- You must include specific endpoints in the allowlist for desktop flows containing cloud connector actions to work properly. For more information, see [Desktop flow services required for runtime](../../ip-address-configuration.md#allow-machines--users-on-your-network-to-access-power-automate-desktop-services).
...
To pass a file as an input to a cloud connector action, first convert the file to binary data by using the [Convert file to binary data](file.md#convertfiletobinaryaction) action.
...
By using connection reference embedding, you can give other co-owners access to your connection references and their underlying resources. You can only do this process in the scope of the respective shared desktop flow.

To embed a connection reference to a flow that you can access as an owner or co-owner:

1. Select the desktop flow in Power Automate (make.powerautomate.com), and then select **Details**.
...
1. In the **Connection references** screen, view all of the connection references used in the flow.
...
> [!NOTE]
> You can only embed or remove connection references that you own. Other co-owners can only manage connection references they add.
...
If you're a co-owner, bring your own connection (BYOC) in console runs for connection references that aren't embedded. If you're a run-only user, bring your own connection, even if the connection references are embedded.
...
>  * The user running the flow also owns the embedded connections, and
...
Power Automate for desktop adds these cloud connectors to desktop flows by default. You always see them in the actions pane:
...
To add other non-custom Power Automate cloud connectors to a desktop flow, use the [Assets library](../assets-library.md#connectors-tab).
...
- You can't create all connection types through Power Automate for desktop. For these connectors, create a connection through the corresponding [portal page](../../add-manage-connections.md#add-a-connection).
