...
| Work queue tables not available for Power BI reporting using [Dataverse](/power-query/connectors/dataverse) connector | The work queue and work queue item tables are not present in TDS-based reporting scenarios today, which is why the Power BI [Dataverse](/power-query/connectors/dataverse) connector (which is based on [TDS](/power-query/connectors/dataverse#prerequisites)) is not listing them. We plan to add them in the future. | Consider using the [Common Data Service (Legacy)](/power-query/connectors/common-data-service-legacy) or [OData Feed](/power-query/connectors/odata-feed) connector to connect to the underlying work queue tables. Learn about some of their [known limitations.](/power-query/connectors/common-data-service-legacy#limitations-and-issues)|
