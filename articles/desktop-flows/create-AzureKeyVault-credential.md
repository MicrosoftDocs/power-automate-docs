...
- If your environment uses a managed identity (part of enterprise policy creation) used by Dataverse to access, for instance, your Azure Data Lake (as described in [Use managed identities for Azure with your Azure data lake storage](https://learn.microsoft.com/en-us/power-apps/maker/data-platform/azure-synapse-link-msi)), this identity will also by used to access your Azure Key Vault (only one enterprise policy can connect to the Dataverse environment simultaneously). In that case, you need to ensure that the managed identity has the appropriate permissions to the Azure Key Vault resource.
