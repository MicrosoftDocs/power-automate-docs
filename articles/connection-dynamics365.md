<properties
    pageTitle="Create a flow with Dynamics 365 (online) | Microsoft Flow"
...
1. Under **Entity Name**, select the entity that you want to listen to, which will act as a trigger initiating the flow.
...
7.	Under **Organization Name**, select the Dynamics 365 instance where you want the flow to create the record. Notice that it doesn’t have to be the same instance where the event is triggered from.

1. Under **Entity Name**, select the entity that will create a record when the event occurs.
...
7.	Under **List ID**, select **inbox**.

1. Under **Title**, select **Subject** in the dynamic content pane.
