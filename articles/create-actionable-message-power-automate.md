...
Authenticated actionable messages enable you to build secure, enterprise-grade workflows by validating message origin and requiring explicit opt-in via trigger conditions. This provides stronger protection against replay and misuse while preserving a seamless, interactive email experience.

## Create an actionable email

1. Sign in to [Power Automate](https://make.powerautomate.com).

1. After the question, *Is this a Power Automate Scenario?*, select **Yes**.
...
1. After the question, **Who can trigger the flow?**, select **Any user in my tenant** or **Specific users in my tenant**.
...
1. Set this condition: `@equals(triggerOutputs()?['headers']?['Provider-Id'], 'your_actionable_email_provider_id')`
    ...
    If the header isn't valid when the flow is triggered, the HTTP call returns *202 Accepted*. It also doesn't run actions and no run history shows for these calls.
