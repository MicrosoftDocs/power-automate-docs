---
title: Extract data from SAP GUI UI with Power Automate | Microsoft Docs
description: Use the pro-code or low-code approach to extract information from SAP GUI UI.
suite: flow
...
# Extract data from SAP GUI UI with Power Automate

When you create or update records in SAP, it generates  status information which includes the newly generated record IDs. SAP displays these status information in the lower part of the SAP UI.
...
1. Under **Save To**, provide the file path and filename where the captured user interactions should be stored.
...
1. Select the **Record Script** button to start the screen capturing process. Every interaction you do now in SAP is captured as repeatable VBScript commands. 
   ...
   >[!NOTE]
   >If you recorded steps and saved to this file before, you will need to confirm if you want to overwrite the file. 

1. Enter transaction code **IW21**, and then press **Enter**.

1. Provide the **Notification type**, and then press **Enter**.
...
1. Back in the **Create PM Notification** **initial** form, you'll notice a new status message in the status bar on the lower left part of the UI.

   ![Screenshot of SAP Notification Id generated after saving a new record ](media/SAP-notification-id-after-saving.png)

1. Select **Exit (Shift+F3)** in the upper toolbar, and then **stop the recording**.
...
If you want to see the contents of the VBScriptOutput, you could use a display message action, similar to the output in the following images.
...
The following steps won't go into detail on how to record or use manual action design to create a new SAP PM Notification record.

<!--confirm the numbers for the sections-->
Please review section 6 or 7 if you need a refresher on how to that.
...
1. Create a notification record and capture the status text that appears once you've selected **Save** in the upper toolbar of the **Create PM Notification** screen.
   ...
   After the item saves, you'll be redirected to the previous screen where you should see a new notification number in the status bar text.
...
1. Select the UI element dropdown, and then select **Add a new UI element** button.
...
1. Select the **SAP Easy Access** window and hover with your mouse over the status bar until a red border labeled **Gui Statusbar** appears. While the border is active, hold down **Ctrl** and then click to select the status bar.
...
1. In **Text to find**, enter **Notification** and under **Replace with**, enter a blank/empty string which is expressed like this: **%''%**
...
1. Add another **Replace text** action and rename the **Variables produced** to **%NotificationId%**, enter "save" as in **Text to find** value, and under **Replace with**, enter a blank string, which is expressed like this: **%''%**
...
1. Search for the **Trim text** action in the **Actions** pane, drag it onto the design canvas, select **%NotificationId%** as **Text to trim** value and rename **Variables produced** to **%TrimmedNotificationID%**.

    ![Screenshot that shows a Trim text action dialog with its parameters specified ](media/trim-text-action-with-parameters.png)

1. Search for the **Display message** action in the **Actions** pane, drag it onto the design canvas. Set **Message box title** and **Message to display** to suit your needs.
...
1. Run the automation that creates a new SAP PM Notification and extracts its newly generated notification Id that can be displayed as below.
